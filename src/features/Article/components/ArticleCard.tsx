...
import { Card, CardContent, CardFooter, CardHeader, CardTitle, type CardProps } from '@/ui/components/Card';
...
export interface ArticleCardProps extends VariantProps<typeof articleCardVariants> {
    category: string;
    date: string;
    title: string;
    author: {
...
export const ArticleCard = ({
    ...
    const cardContent = (
        ...
        >
            ...
            >
                ...
                <span className={cx("font-mono text-xs", isInverted && "text-background/80")}>
                    {date}
                </span>
            </CardHeader>

            <CardContent className={cx("flex-grow flex flex-col", isCompact ? "p-3" : "p-4")}>
                <CardTitle
                    className={cx(
            ...
            <CardFooter className={cx(isCompact ? "p-3 pt-0" : "p-4 pt-0")}>
                <div className="flex items-center gap-2">
                    <span className={cx("font-mono font-bold", isCompact ? "text-[10px]" : "text-xs")}>
                        {author.name}
                    </span>
                </div>
            </CardFooter>
    ...
    return (
        <article className={baseClassName} onClick={onClick}>
            {cardContent}
        </article>
    );
};
