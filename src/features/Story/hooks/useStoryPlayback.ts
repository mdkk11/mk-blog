...
export const useStoryPlayback = (
    ...
    }, [selectedSetIndex, currentStoryIndex, storySets, options.loop, onNavigateSet, onClose]);
    ...
    // 進捗完了監視
    useEffect(() => {
        if (progress >= 100 && selectedSetIndex !== null && options.autoPlay && !isPaused) {
            next();
        }
